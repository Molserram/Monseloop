...
- [x] Folio indicator (`01 / 05 — Hero` style vertical tags) — user chose to remove it rather than add the missing markup elsewhere. Removed the `<div class="folio">` from `index.html`/`es/index.html`, and stripped the now-dead `.folio` CSS and folio JS (IntersectionObserver, `folioLabels`, `renderFolio`) from all 10 pages.
